...
# ClaudeGo Installation Script for Linux
# Installs VSCode and Claude Code extension on Linux
...
check_linux() {
    ...
    if [[ "$OSTYPE" != "linux-gnu"* ]]; then
        ...
        print_info "For macOS, use ./install.sh instead"
        exit 1
    fi
...
install_system_packages() {
    ...
    # Install curl, wget, ca-certificates if not present
...
install_node() {
    ...
    print_info "Installing Node.js and npm via apt..."

    # Install Node.js LTS from NodeSource repository for better version
...
install_vscode() {
    ...
    # Add code command to PATH if needed
...
install_claude_extension() {
    ...
    if code --install-extension anthropics.claude-code --force; then
        ...
        print_info "2. Press Ctrl+Shift+X"
...
main() {
    ...
    echo -e "${GREEN}╔═══════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║                                                   ║${NC}"
    echo -e "${GREEN}║      ClaudeGo Installation Script (Linux)         ║${NC}"
    echo -e "${GREEN}║                                                   ║${NC}"
    echo -e "${GREEN}╚═══════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    print_info "  ✓ System packages (curl, wget, gnupg)"
    ...
    echo ""
    print_info "Run './verify-linux.sh' to verify your installation"
    echo ""
}
